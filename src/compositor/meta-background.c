  ...
  switch (priv->style)
    {
      ...
      case C_DESKTOP_BACKGROUND_STYLE_ZOOM:
          ...
          if ((priv->style == C_DESKTOP_BACKGROUND_STYLE_SCALED &&
                (monitor_x_scale < monitor_y_scale)) ||
              (priv->style == C_DESKTOP_BACKGROUND_STYLE_ZOOM &&
                (monitor_x_scale > monitor_y_scale)))
            {
...
meta_background_load_still_frame (MetaBackground *self)
{
  ...
  unset_texture (self);
  set_style (self, C_DESKTOP_BACKGROUND_STYLE_STRETCHED);

  still_frame = get_still_frame_for_monitor (priv->screen, priv->monitor);
  XSync (meta_display_get_xdisplay (display), False);

  meta_error_trap_push (display);
  texture = texture = COGL_TEXTURE (cogl_texture_pixmap_x11_new (context, still_frame, FALSE, &error));
                               ...
                               ClutterColor               *second_color)
{
  ...
  unset_texture (self);
  set_style (self, C_DESKTOP_BACKGROUND_STYLE_NONE);
 ...
 * @style: a #CDesktopBackgroundStyle to specify how background is laid out
 ...
 *
 * Return value: a #CDesktopBackgroundStyle
 */
 ...
 *
 * Return value: a #CDesktopBackgroundShading
 */
