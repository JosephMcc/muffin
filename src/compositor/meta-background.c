...
need_prerender (MetaBackground *self)
{
  ...
  if (texture2 == NULL && priv->style == C_DESKTOP_BACKGROUND_STYLE_WALLPAPER)
    return FALSE;
  ...
  if (priv->color_texture == NULL)
    {
      ...
      if (priv->shading_direction == C_DESKTOP_BACKGROUND_SHADING_SOLID)
        {
          width = 1;
          height = 1;
                             ...
                             CoglPipelineWrapMode   *wrap_mode)
{
  ...
  if (texture2 == NULL && priv->style == C_DESKTOP_BACKGROUND_STYLE_WALLPAPER &&
      priv->shading_direction == C_DESKTOP_BACKGROUND_SHADING_SOLID &&
