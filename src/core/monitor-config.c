...
meta_monitor_config_init (MetaMonitorConfig *self)
{
  ...
  filename = g_getenv ("MUFFIN_MONITOR_FILENAME");
  if (filename == NULL)
                     ...
                     int                max_height)
{
  ...
  /* If we reach this point, this is either the first time muffin runs
     ...
     or in muffin core, so let's do extended for now.
  */
  x = 0;
  y = 0;
  for (i = 0; i < n_outputs; i++)
  ...
  while (g_hash_table_iter_next (&iter, (gpointer*) &config, NULL))
    {
      /* Note: we don't distinguish clone vs non-clone here, that's
         something for the UI (ie cinnamon-control-center) to handle,
