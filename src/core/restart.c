...
meta_restart_init (void)
{
  ...
  Atom atom_restart_helper = XInternAtom (xdisplay, "_MUFFIN_RESTART_HELPER", False);
 ...
 *
 * Starts the process of restarting the compositor. Note that Muffin's
...
meta_restart (const char *message)
{
  ...
  if (!g_spawn_async_with_pipes (NULL, /* working directory */
                                 ...
                                 &error))
    {
      g_printerr("Failed to start the restart helper\n");
 ...
 * meta_is_restart:
 *
 * Returns %TRUE if this instance of Muffin comes from Muffin
