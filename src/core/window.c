          ...
          if (meta_grab_op_is_moving (window->display->grab_op))
            {
              if (window->tile_mode != META_TILE_NONE &&
                  ...
                  update_move (window,
                               ...
                               xev->mods.effective & get_mask_from_snap_keysym (window),
                               xev->root_x,
                               xev->root_y);
              if (meta_prefs_get_edge_tiling ())
          ...
          if (xev->root == window->screen->xroot)
            {
              if (check_use_this_motion_notify (window,
                ...
                KeySym keysym = XkbKeycodeToKeysym (window->display->xdisplay, xev->detail, 0, 0);
                if (mod_set[0] != 0)
                {
                    gboolean snap = FALSE;
                    if (xev->evtype == XI_KeyPress && (keysym == mod_set[0] ||
                                                       keysym == mod_set[1]))
                        snap = TRUE;
                    update_move (window,
                                 xev->mods.effective & ShiftMask,
                ...
                if (xev->evtype == XI_KeyPress && keysym == XK_Left) {
                ...
                if (xev->evtype == XI_KeyPress && keysym == XK_Right) {
                    MetaWorkspace *target_workspace = meta_workspace_get_neighbor (window->screen->active_workspace,
                                                                                   motion_right);
                    if (target_workspace)
                      {
